...
export function countTrailingMistypes(
	history: RuntimeStats["keyStabilityHistory"],
) {
	let count = 0;
...
export function shouldAutoRetireSession({
	...
	stats: Pick<
		RuntimeStats,
		"characterAttempts" | "keyStabilityHistory" | "lastInputAt"
	>;
}) {
	...
	if (
		...
		countTrailingMistypes(stats.keyStabilityHistory) >=
			settings.consecutiveMistypeRetireCount
	) {
		return true;
	}
...
export function useTypingSession({
	...
	const [challengeLanguage, setChallengeLanguage] = useState<ChallengeLanguage>(
		initialChallengeLanguage,
	);
	const [productionDuration, setProductionDuration] =
		useState<ProductionDuration>(300);
	...
	const [nextProductionChallengeOrder, setNextProductionChallengeOrder] =
		useState(() => initialProductionChallengeOrder);
	...
	const durationSeconds =
		mode.group === "production" ? productionDuration : mode.durationSeconds;
	const elapsedSeconds = startedAt
		? Math.min((now - startedAt) / 1000, durationSeconds)
		: 0;
	...
	const nextDirectChallenge =
		directChallenges[nextDirectChallengeIndex] ?? null;
	const imeChallenges =
		challengeLanguage === "ja" ? longChallenges : englishLongChallenges;
	const currentImeChallenge = imeChallenges[directChallengeIndex] ?? "";
	const currentDirectGuideSource =
		currentDirectChallenge.guide ?? currentDirectChallenge.input;
	...
	const currentFurigana =
		challengeLanguage === "ja"
			? mode.requiresIme
				? (longChallengeFurigana[
						directChallengeIndex % longChallengeFurigana.length
					] ?? [])
	...
	const currentRawReading =
		challengeLanguage === "ja"
			? mode.requiresIme
				? (longChallengeReadings[
						directChallengeIndex % longChallengeReadings.length
					] ?? "")
	...
	const currentProductionPlayable =
		mode.group === "production"
			? productionPlayableModes[mode.id as ProductionModeId]
			: true;

	function randomizePracticeChallengeOrder(
		language: ChallengeLanguage = challengeLanguage,
	) {
		const nextOrder = createShuffledIndexes(
			getDirectChallenges(language, "practice").length,
		);
	...
	function randomizeProductionChallengeOrder(
		language: ChallengeLanguage = challengeLanguage,
	) {
		const nextOrder = createShuffledIndexes(
			getDirectChallenges(language, "production").length,
		);
	...
	function randomizeChallengeOrders(
		language: ChallengeLanguage = challengeLanguage,
	) {
		...
		() =>
			calculateMetrics({
				...
				scoreDurationSeconds:
					stored.settings.rankCalculationMode === "actual"
						? durationSeconds
						: undefined,
	...
	const isProductionBlocked =
		mode.group === "production" &&
		!canPlayProductionMode({
			modeId: mode.id as ProductionModeId,
	...
	const correctionDebt =
		!acceptsTextInput && mode.lockMistakes ? stats.mistakeDebt : 0;
	...
	function finishSession(reason: FinishReason = "completed") {
		...
		const sessionScore =
			reason === "retired"
				? applyAutoRetireScorePenalty(metrics.score)
				: metrics.score;
	...
	function recordKey(
		...
		setStats((previous) => {
			const intervalMs =
				previous.lastKeyAt === null ? null : timestamp - previous.lastKeyAt;
	...
	function handleDirectKeyDown(event: DirectKeyEvent) {
		...
		setStats((previous) => ({
			...
			lastKeyAt:
				result.state.completedPrompts > previous.completedPrompts
					? null
					: previous.lastKeyAt,
		}));
	...
	function handleImeKeyDown(event: ReactKeyboardEvent<HTMLTextAreaElement>) {
		...
		if (!matches) {
			playTypingSound("mistake");
			setImeError(
				"課題文と一致していません。修正してから Enter で提出してください。",
			);
			setStats((previous) => ({
				...previous,
	...
	return {
		...
		typingPanelProps: {
			...
			currentGuide:
				currentGuide ??
				(typeof currentInputTarget === "string"
					? currentInputTarget
					: currentInputTarget.guide),
			currentRomajiTarget,
			currentRank,
			finishReason,
			imeError,
